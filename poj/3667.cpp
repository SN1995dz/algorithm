...
class segtree {
    public:
        ...
        int find_first_knowingly(int x, int l, int r, const function<bool(const int&, const int&)> &f) {
            if (l == r) {
                return l;
            }
            push(x, l, r);
            ...
            int z = x + ((y - l + 1) << 1);
            int res;
            if (f(l, r)) res = find_first_knowingly(x + 1, l, y, f);
            else res = find_first_knowingly(z, y + 1, r, f);
            pull(x, z);
            return res;
        }

        int find_first(int x, int l, int r, int ll, int rr, const function<bool(const int&, const int&)> &f) {
            if (ll <= l && r <= rr) {
                if (!f(l, r)) return -1;
        ...
        int find_first(int ll, int rr, const function<bool(const int&, const int&)> &f) {
            assert(0 <= ll && ll <= rr && rr <= n - 1);
...
int main() {
    ...
    while (m--) {
        ...
        if (op == 1) {
            int d;
            cin >> d;
            int p = st.find_first(0, n - 1, [&](const int &l, const int &r) {
                return r - l + 1 >= d && st.get(l, r).val == r - l + 1;
            });
            cout << p + 1 << endl;
            st.modify(p, p + d - 1, 0);
        } else {
