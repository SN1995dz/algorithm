...
int main() {
    ...
    while (T--) {
        ...
        sparse_table<int> st(H), rst(rH);
        vector<int> a(n + 1, 0), b(n + 1, 0);
        for (int l = 1; l <= n / 2; ++l) {
            for (int i = l; i < n; i += l) {
                int L = rk[i], R = rk[i - l];
                if (L > R) swap(L, R);
                int right = st.get(L + 1, R);
                right = min(right, l);
                L = rrk[n - 1 - i], R = rrk[n - 1 - (i - l)];
                if (L > R) swap(L, R);
                int left = rst.get(L + 1, R);
                left = min(left, l);
                if (left + right - 1 >= l) {
                    ++a[i - l - left + 1];
                    --a[i - l - left + 1 + left + right - 1 - l + 1];
                }
                L = rrk[i], R = rrk[i - l];
                if (L > R) swap(L, R);
                right = rst.get(L + 1, R);
                right = min(right, l);
                L = rk[n - 1 - i], R = rk[n - 1 - (i - l)];
                if (L > R) swap(L, R);
                left = st.get(L + 1, R);
                left = min(left, l);
                if (left + right - 1 >= l) {
                    ++b[n - 1 - (i - l - left + 1 + left + right - 1 - l)];
                    --b[n - 1 - (i - l - left + 1) + 1];
                }
            }
        }
